...
def Objective(trial):        
        ...
        #rotation=trial.suggest_float("rotation", 0, 50)
        #width_shift=trial.suggest_float("w_shift", 0, 0.5)
        #height_shift=trial.suggest_float("h_shift", 0, 0.5)
        #shear=trial.suggest_float("shear", 0, 0.5)
        #zoom=trial.suggest_discrete_uniform("zoom", 0.05, 0.2, 0.05)
        BATCH_SIZE= trial.suggest_categorical('batch_size',[10,20,40,60,80,100])
                          
        #dict_params = {'rotation':rotation,
                       #'w_shift':width_shift,
                       #'h_shift':height_shift,
                       #'shear':shear,
                       #'zoom':zoom,}
        ...
        plot_model(model, to_file='modelNASNetMobile_3capas_si_aug_solo_batchnorm_small_training_optuna.png', show_shapes=True, show_layer_names=True)

        # compile the model (should be done *after* setting layers to non-trainable)
        model.compile(loss='categorical_crossentropy',optimizer='adam', metrics=['accuracy'])

        WEIGHT_PATH = './3capas_si_aug_solo_batchnorm_small_training_optuna.h5'
...
if True:
    ...
    plt.savefig('accuracy_3capas_si_aug_solo_batchnorm_small_training_optuna.jpg')
    plt.close()
    ...
    plt.xlabel('epoch')
    plt.legend(['train', 'validation'], loc='upper left')
    plt.savefig('loss_3capas_si_aug_solo_batchnorm_small_training_optuna.jpg')
